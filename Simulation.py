...
def calculateCloud2(uaw: Uaw, walls):
    t = np.array([])
    rows = len(uaw.matrixDist)
    cols = len(uaw.matrixDist[0])
    DistanceMatrix = np.zeros(shape=(rows, cols, 4))
...
def calculateDist4(walls, vec: DistVector, Az=0, fi=0,  gamma=0, mu=0, std=0.02):
    t = []
    acc = []
    index = 0
    A3 = np.array([
